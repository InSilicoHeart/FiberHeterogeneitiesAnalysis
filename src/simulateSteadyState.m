...
if(isempty(dir([pathSS '/control'])))
    ...
    a=load('post/SS_prc0_00000151.var');
    dt_results = a(2,1)-a(1,1);
    V=zeros(length(a(:,2)),5);
    V(:,1)=a(:,2);
    a=load('post/SS_prc0_00000176.var');
    V(:,2)=a(:,2);
    a=load('post/SS_prc0_00000201.var');
    V(:,3)=a(:,2);
    a=load('post/SS_prc0_00000226.var');
    V(:,4)=a(:,2);
    a=load('post/SS_prc0_00000251.var');
    V(:,5)=a(:,2);
    for i=1:5
        [APD90{i},APD_time{i}]= calculateAPD(V(:,i),dt_results);
    end
...
for i=1:length(param)
  ...
  for j=1:length(values)
    ...
    APD_time_j = cell(1,5);
    for k=1:5
        [APD90{k},APD_time{k}]= calculateAPD(V(:,k),dt_results);
    end
